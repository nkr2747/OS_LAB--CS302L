...
void LRU_local(int pageSize, int totFrames, string sourceFile)
{
    ...
    while (getline(file, line, ','))
    {
        ...
        if (!proc[p_no].isMapped(page_no))
        {
            ...
            int frame_no = freeFrames[p_no].front();
            freeFrames[p_no].pop();
            proc[p_no].pageTable[page_no] = frame_no;
...
void OPT_global(int pageSize, int totFrames, string sourceFile)
{
    ...
    for (int idx = 0; idx < (int)accesses.size(); ++idx)
    {
        ...
        if (!proc[p_no].isMapped(page_no))
        {
            ...
            // If no free frame, choose victim using OPT (global)
            if (freeFrames.empty())
            {
                //cout<<".";
                int victim_p = -1;
                int64_t victim_page = -1;
                int best_next_use = -1; // we want max next_use
                int flag = false;
                for(int i = 0; i < 4; i++){
                    for(auto it: proc[i].pageTable){
                        
                        int tp_no = i;
                        int64_t tpage_no = it.first;
                        int next = INT_MAX;
                        //cout<<".";
                        if(futurePos.find({tp_no,tpage_no}) != futurePos.end() && !futurePos[{tp_no,tpage_no}].empty()){
                            next = futurePos[{tp_no,tpage_no}].front();
                        }
                        
                        if(next == INT_MAX){
                            victim_p = tp_no;
                            victim_page = tpage_no;
                            flag = true;
                            break;
                        }
                        //cout<<".";
                        if(next > best_next_use){
                            victim_p = tp_no;
                            victim_page = tpage_no;
                            best_next_use = next;
                        }
...
void OPT_local(int pageSize, int totFrames, string sourceFile)
{
    ...
    string outname = string("result_OPT_local_") + to_string(pageSize) + "_" + to_string(totFrames) + ".txt";
    ofstream output(outname);
    for (int idx = 0; idx < (int)accesses.size(); ++idx)
    {
        auto cur = accesses[idx];
        ...
        if (!proc[p_no].isMapped(page_no))
        {
            ...
            if (freeFrames[p_no].empty())
            {
                ...
                for( auto it: proc[p_no].pageTable){
                    int64_t cand_page = it.first;
                    int next = INT_MAX;
                    if(futurePos.find({p_no,cand_page}) != futurePos.end() && !futurePos[{p_no, cand_page}].empty()){
                        next = futurePos[{p_no, cand_page}].front();
                    }
                    if(next == INT_MAX){
                        victim_page = cand_page;
                        break;
                    }
                    if(next > best_next_use){
                        best_next_use = next;
